...
import {
  Heart, ChatCircle, Repeat2, ShareNetwork, SpinnerGap, Image as ImageIcon,
  X, SealCheck, DotsThreeHorizontal, PencilSimple, Trash, BookmarkSimple
...
function Avatar({ src, name, size = 40, gold }: { src?: string; name: string; size?: number; gold?: boolean }) {
  ...
  return (
    <div
      className={`rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-white font-bold flex-shrink-0 ${cls}`}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-1 flex-wrap">
              ...
              {post.isVerified && <SealCheck size={16} weight="fill" className="text-blue-400 flex-shrink-0" />}
            ...
            {isAuthor && (
              <div className="relative">
                <button onClick={() => setShowMenu(!showMenu)} className="text-gray-600 hover:text-blue-400 transition-colors p-1 rounded-full hover:bg-blue-400/10 flex-shrink-0">
                  <DotsThreeHorizontal size={18} weight="bold" />
                </button>
                {showMenu && (
          ...
          {!editing && (
            <div className="flex items-center justify-between mt-3 max-w-[300px] text-gray-500">
              ...
              <button onClick={() => onBookmark?.(post._id)} className="flex items-center gap-1.5 hover:text-yellow-400 transition-colors group">
              ...
              <form onSubmit={submitComment} className="flex gap-2 mb-3">
                ...
                <div className="flex-1 flex gap-2">
                  ...
                  <button type="submit" disabled={!comment.trim() || commenting} className="bg-blue-500 text-white px-3 py-1 rounded-full text-xs font-bold disabled:opacity-50 hover:bg-blue-600 transition-all">
                    {commenting ? <SpinnerGap size={12} className="animate-spin" /> : "Reply"}
                  </button>
                </div>
              </form>
            ...
            <div className="flex gap-2 mt-2 flex-wrap">
              {images.map((img, i) => (
                <div key={i} className="relative">
                  ...
                  <button type="button" onClick={() => setImages(prev => prev.filter((_, j) => j !== i))} className="absolute -top-2 -right-2 bg-black rounded-full p-0.5 hover:bg-slate-800">
          ...
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-700/50">
            ...
            <div className="flex items-center gap-3">
              ...
              <button type="submit" disabled={(!content.trim() && !images.length) || posting || content.length > 280}
                ...
                {posting ? <SpinnerGap size={16} className="animate-spin" /> : "Post"}
              </button>
            </div>
          </div>
        </div>
