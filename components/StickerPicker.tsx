...
export default function StickerPicker({ onSelectSticker, onSelectEmoji, onClose }: Props) {
  ...
  useEffect(() => {
    if (tab === "stickers" && packs.length === 0) {
      setLoading(true);
      fetch("/api/stickers", { credentials: "include" })
        ...
        .then(d => {
          if (d.packs) {
            setPacks(d.packs);
            const first = d.packs.find((p: Pack) => p.owned);
            if (first) setActivePack(first.id);
          }
        })
        .catch(() => {})
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {loading ? (
              <div className="flex items-center justify-center h-full">
                <SpinnerGap size={20} className="animate-spin text-white/40" />
              </div>
            ) : !currentPack ? (
                ...
                {packs.filter(p => !p.owned).map(pack => (
                  <div key={pack.id} className="flex items-center gap-3 p-2 rounded-xl border border-white/10">
                    ...
                    <button onClick={() => buyPack(pack)} disabled={purchasing === pack.id}
                      className="flex items-center gap-1 text-[10px] font-bold px-2 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50">
                      {purchasing === pack.id ? <SpinnerGap size={10} className="animate-spin" /> : <ShoppingCart size={10} />}
                      Buy
                    </button>
                  </div>
                ))}
              ...
              <div className="flex flex-col items-center justify-center h-full gap-3 px-2">
                ...
                <button onClick={() => buyPack(currentPack)} disabled={purchasing === currentPack.id}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white font-semibold text-sm hover:bg-blue-700 disabled:opacity-50">
              ...
              <div className="grid grid-cols-4 gap-1">
                {currentPack.stickers.map(s => (
                  <button key={s.id} onClick={() => onSelectSticker(s.value)}
                    ...
                    {s.value}
                    {s.isAnimated && <span className="absolute bottom-0.5 right-0.5 text-[7px] bg-purple-600 text-white rounded px-0.5 leading-none">✦</span>}
                  </button>
                ))}
              </div>
            )}
