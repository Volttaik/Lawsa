...
export default function ClansClient({ initialClans, currentUser: initialUser }: { initialClans: Clan[]; currentUser: CurrentUser | null }) {
  ...
  /* ── Dedup refs (fixes the infinite-recreate + duplicate-message bug) ── */
  ...
  /* ── Stable poll — deps: [] so the interval never gets recreated ── */
  const pollChat = useCallback(async () => {
    ...
    try {
      ...
      if (data.messages?.length) {
        /* Filter messages we've already seen */
  ...
  const openClan = async (clan: Clan) => {
    /* Reset dedup state for the new clan */
    ...
    /* Seed the seen-set so the poller never re-adds these */
    initialMsgs.forEach(m => seenIdsRef.current.add(m._id));
    if (initialMsgs.length) {
      lastMsgTimeRef.current = initialMsgs[initialMsgs.length - 1].createdAt;
    }
  ...
  const handleSendChat = async () => {
    ...
    if (data.message) {
      ...
      /* Add to seen-set and update ref so poller skips it */
                          ...
                          <div key={msg._id} className={`flex items-end gap-2 ${isMine ? "flex-row-reverse" : ""} ${i > 0 && prevMsg?.senderId === msg.senderId ? "mt-0.5" : "mt-3"}`}>
                            ...
                            <div className="max-w-[75%]">
                              ...
                              <div className={`px-3 py-2 text-white ${isMine ? "rounded-2xl rounded-br-sm" : "rounded-2xl rounded-bl-sm"}`}
                                ...
                                <StickerRenderer content={msg.content} className="text-[14px] leading-relaxed" />
                              </div>
                              {isLast && (
