...
export default function FollowingPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const [meRes, profileRes, followingRes] = await Promise.all([
  ...
  return (
    <div className="max-w-[600px] mx-auto border-x border-[#2f3336] min-h-screen bg-black">
      <div className="sticky top-0 z-10 bg-black/80 backdrop-blur-md px-4 py-3 flex items-center gap-4 border-b border-[#2f3336]">
        ...
        <div>
          <h1 className="text-lg font-bold text-white">{profileName || "Following"}</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center px-4">
          <p className="text-gray-300 font-semibold mb-1">Not following anyone yet</p>
          <p className="text-gray-600 text-sm">When this account follows someone, they&apos;ll appear here.</p>
        </div>
      ) : (
        <div>
          {users.map(user => {
            ...
            return (
              <div key={user._id} className="flex items-center gap-3 px-4 py-3 border-b border-[#2f3336] hover:bg-white/[0.03] transition-colors">
                ...
                {!isMe && (
                  ...
                  >
                    {isLoading ? <SpinnerGap className="w-3.5 h-3.5 animate-spin" /> : isFollowingUser ? <UserCheck className="w-3.5 h-3.5" /> : <UserPlus className="w-3.5 h-3.5" />}
