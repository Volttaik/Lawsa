...
function MessagesPageInner() {
  ...
  const send = async (e: React.FormEvent) => {
    ...
    const tempMsg: Message = {
      ...
      content: savedText,
      mediaUrl: savedAudioData || savedMediaData || "",
      mediaType: savedAudioData ? "audio" : savedMediaData?.startsWith("data:image") ? "image" : savedMediaData?.startsWith("data:video") ? "video" : "",
      read: false,
      edited: false,
      isDeleted: false,
    ...
    else if (savedMediaData) { body.mediaData = savedMediaData; body.mediaType = savedMediaData.startsWith("data:image") ? "image" : savedMediaData.startsWith("data:video") ? "video" : "file"; }
  ...
  const ChatWindow = (active || pendingRecipient) ? (
    <div className="relative flex flex-col h-screen min-h-0 overflow-hidden" style={{ backgroundColor: "#000000" }}>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-30" style={{ backgroundColor: "#000000" }}>
        ...
        {/* Image preview — proper full-width panel above input */}
        {mediaPreview && (
          <div className="relative w-full border-t" style={{ backgroundColor: "#0d0d0d", borderColor: "#1e1e1e" }}>
            <div className="flex items-end gap-3 px-4 py-3">
              <div className="relative flex-shrink-0">
                ...
                <button onClick={() => { setMediaData(null); setMediaPreview(null); }}
                  className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full flex items-center justify-center"
                  style={{ backgroundColor: "rgba(0,0,0,0.65)", backdropFilter: "blur(4px)" }}>
                  <X className="w-3.5 h-3.5 text-white" />
                </button>
              </div>
              <p className="text-xs pb-2 leading-relaxed" style={{ color: "rgba(255,255,255,0.4)" }}>
                Image ready to send.<br />Add a caption below.
              </p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={send} className="flex items-end gap-2" style={noOutline}>
          ...
          {!isRecording && (
            <button
              type={text.trim() || mediaData || audioPreviewUrl ? "submit" : "button"}
