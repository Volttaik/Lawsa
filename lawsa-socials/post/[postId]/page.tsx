...
export async function generateMetadata({ params }: { params: Promise<{ postId: string }> }): Promise<Metadata> {
  try {
    ...
    if (!post) return { title: "Post not found — LAWSA" };
    const headersList = await headers();
    const host = headersList.get("host") || "lawsa.app";
    const baseUrl = `https://${host}`;
    const authorHandle = post.authorUsername ? `@${post.authorUsername}` : post.authorName;
    const description = post.content?.slice(0, 200) || "View this post on LAWSA";
    const title = `${authorHandle} on LAWSA`;
    const images: string[] = post.images || [];
    const ogImage = images[0]?.startsWith("http") ? images[0] : `${baseUrl}/logo.png`;
    return {
      title,
      description,
      openGraph: { title, description, url: `${baseUrl}/post/${postId}`, siteName: "LAWSA", images: [{ url: ogImage }], type: "article" },
      twitter: { card: "summary_large_image", title, description, images: [ogImage] },
    };
  } catch {
    return { title: "LAWSA — Connect, Share & Grow" };
  }
}
