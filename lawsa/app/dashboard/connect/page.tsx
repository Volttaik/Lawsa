...
function ConnectPageContent() {
  ...
  const handleFollow = async (userId: string) => {
    ...
    setFollowing(prev => {
      ...
      if (wasFollowing) s.delete(userId); else s.add(userId);
      return s;
    });
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setFollowing(prev => {
          const s = new Set(prev);
          if (wasFollowing) s.add(userId); else s.delete(userId);
          return s;
        });
      } else {
        const data = await res.json();
        ...
        if (typeof data.following === "boolean") {
          setFollowing(prev => {
            const s = new Set(prev);
            if (data.following) s.add(userId); else s.delete(userId);
            return s;
          });
        }
      }
...
export default function ConnectPage() {
  return (
    <Suspense fallback={<div className="flex justify-center py-16"><Loader2 className="animate-spin text-blue-500" size={28} /></div>}>
      <ConnectPageContent />
    </Suspense>
  );
}
